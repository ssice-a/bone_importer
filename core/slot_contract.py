...
Slot ids are runtime namespace ids.  Coordinate mirror flags may change vector
values during geometry export, but they must not silently swap slot semantics.
If a DrawPart needs non-identity slot binding, it must use an explicit Bone
Slot Map JSON.
"""
