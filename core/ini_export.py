...
def _append_bone_texture_override(lines: list[str], export_result):
    mesh_key = _infer_mesh_key_from_clip_path(export_result.static_clip_path)
    ...
    _write_line(lines, "    ; If this draw needs a special CB1 flag (for example eyelashes), bind cs-t3 before RedirectCB1.")
...
def _append_morph_texture_override(lines: list[str], export_result, morph_result):
    mesh_key = morph_result.mesh_key
...
def write_generated_runtime_ini(output_directory: str, clip_name: str, export_results, morph_results=()):
    ...
    if normalized_export_results:
        for export_result in normalized_export_results:
            ...
            if morph_result is None:
                _append_bone_texture_override(lines, export_result)
            else:
                _append_morph_texture_override(lines, export_result, morph_result)
    else:
