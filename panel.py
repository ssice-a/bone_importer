...
class VIEW3D_PT_bone_importer(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        selected_proxy_armature_count = len(list_selected_proxy_armatures(context))
        ...
        if proxy_armature is not None:
            ...
            if proxy_armature.bi_part_id >= 0:
                ...
                binding_box.label(text=f"Part Size: {part_layout['part_size']}", icon="INFO")
                binding_box.label(text=f"Previous Base: {part_layout['previous_base']}", icon="INFO")
                binding_box.label(text=f"Previous Offset: {part_layout['previous_offset']}", icon="INFO")
        ...
        io_row = workflow_box.row(align=True)
        io_row.operator("object.bi_export_palette", icon="EXPORT")
        io_row.operator("object.bi_import_palette", icon="IMPORT")
        io_row.enabled = (
        ...
        if selected_proxy_armature_count > 1:
            export_box.label(text=f"Selected Proxy Armatures: {selected_proxy_armature_count}", icon="INFO")
        ...
        if selected_proxy_armature_count > 1:
            import_box.label(text=f"Selected Proxy Armatures: {selected_proxy_armature_count}", icon="INFO")
